...
const changePage = (currentPage: number | string | undefined, offset: number, total: number) => {
...
const NewsPage: React.SFC<Props> = (props): JSX.Element => {
    ...
    const nextPage = React.useCallback(() => changePage(page, 1, total), [page, total])

    const prevPage = React.useCallback(() => changePage(page, -1, total), [page, total])
        ...
        <div>
            {
             ...
             <ul>
              ...
              <li>{page}</li>
              <li>
                <button type="button" onClick={nextPage} disabled={Number(page) >= total}>→</button>
              </li>
            </ul>
        </div>
    )
